...
$$
We want to look at three models for $\log(\theta_i)$.
...
mod1 = stan(data = stan_data, file = here("9q2m1.stan"))
```

```{r, warning=FALSE, message=FALSE, results='hide'}
# Model 2
mod2 = stan(data = stan_data, file = here("9q2m2.stan"))
```

```{r, warning=FALSE, message=FALSE, results='hide'}
# Model 3
mod3 = stan(data = stan_data, file = here("9q2m3.stan"))
```
